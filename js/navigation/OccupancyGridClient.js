...
OccupancyGridClient.prototype.unsubscribe = function(){
  if(this.rosTopic){
    this.rosTopic.unsubscribe(this.processMessage);
  }
};

OccupancyGridClient.prototype.subscribe = function(){
  this.unsubscribe();
...
OccupancyGridClient.prototype.processMessage = function(message){
  // check for an old map
  if (this.currentGrid) {
    // check if it there is a tf client
    if (this.tfClient) {
      // grid is of type SceneNode
  ...
  if (this.tfClient) {
    ...
    if (this.sceneNode === null) {
      ...
      this.rootObject.add(this.sceneNode.model_object);
    } else {
      this.sceneNode.add(this.currentGrid);
    }
  } else {
    this.sceneNode = this.currentGrid = newGrid;
    this.rootObject.add(this.currentGrid.mesh);
  }
