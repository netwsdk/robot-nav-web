...
LaserScan.prototype.unsubscribe = function(){
  if(this.rosTopic){
    this.rosTopic.unsubscribe(this.processMessage);
  }
};

LaserScan.prototype.subscribe = function(){
  this.unsubscribe();
...
LaserScan.prototype.processMessage = function(message){
  ...
  for(var i=0;i<n;i+=this.points.pointRatio){
    ...
    if(range >= message.range_min && range <= message.range_max){
        ...
        this.points.positions.array[j+1] = range * Math.sin(angle);
        this.points.positions.array[j+2] = 0.0;

        // console.log("point[" + (j/3) + "]: x=" + this.points.positions.array[j] + " y=" + this.points.positions.array[j+1] + " z=" + this.points.positions.array[j+2]);
        j+=3;
    }
  }
